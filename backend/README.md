...
Open `http://127.0.0.1:5000` in the browser. Flask serves both the frontend and
the API, so do not open `frontend/index.html` directly with `file://`.

The frontend sends `POST /api/emergency` when processing an emergency and
`POST /api/chat` when sending an SOS. If the frontend is served separately,
define `window.API_BASE_URL` before `script.js` loads, for example:

```html
<script>window.API_BASE_URL = "http://127.0.0.1:5000/api";</script>
